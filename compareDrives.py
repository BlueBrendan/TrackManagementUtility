    ...
    for var in first_directory_files:
        ...
        if os.path.isdir(str(first_directory) + '/' + str(var)) and complete==False:
            ...
            if var not in second_directory_files:
                ...
                popup.config(bg=bg)
                if len([name for name in os.listdir(first_directory + '/' + var)]) == 1: tk.Label(popup, text=var + "  (1 file inside)", wraplength=500, font=('Proxima Nova Rg', 13), fg="white", bg=bg).pack(pady=(20, 20))
                else: tk.Label(popup, text=var + "  (" + str(len([name for name in os.listdir(first_directory + '/' + var)])) + " files inside)", wraplength=500, font=('Proxima Nova Rg', 13), fg="white", bg=bg).pack(pady=(20, 20))
                tk.Label(popup, text="Found in " + first_directory, wraplength=500, font=('Proxima Nova Rg', 11), fg="white", bg=bg).pack()
                tk.Label(popup, text="Not found in " + second_directory, wraplength=500, font=('Proxima Nova Rg', 11), fg="white", bg=bg).pack(pady=(20, 20))
                buttonFrame = tk.Frame(popup, bg=bg)
                buttonFrame.pack(pady=(35, 0))
