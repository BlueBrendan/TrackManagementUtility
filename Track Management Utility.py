...
def readValuesFromConfig(CONFIG_FILE):
    config_file = open(CONFIG_FILE, 'r').read()
    terms = ["Subdirectories (B)", "Close Scraping Window (B)", "First Default Directory (S)", "Second Default Directory (S)", "Scrape Junodownload (B)", "Scrape Beatport (B)", 'Scrape Discogs (B)', "Extract Image from Website (B)", "Reverse Image Search (B)", "Delete Stored Images (B)", "Image Load Wait Time (I)", "Stop Search After Conditions (B)", "Stop Search After Finding Image of Resolution (S)", "Scan Filename and Tags (B)", "Check for Numbering Prefix (B)", "Check for Extraneous Hyphens (B)", "Check for Capitalization (B)", "Always Capitalize (L)", "Never Capitalize (L)", "Audio naming format (S)", "Selected Tags (L)", "Delete Unselected Tags (B)", "Calculate ReplayGain (B)", "Overwrite existing ReplayGain value (B)"]
    options = {}
    for term in terms:
